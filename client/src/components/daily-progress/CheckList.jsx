...
export default function CheckList({ habits, setHabits, name, toggleCompletion }) {
  ...
  function renderListItems() {
    return Object.keys(habits).map(i => {
      return (
        <ListItem key={i} i={i} {...habits[i]} setHabits={setHabits} toggleCompletion={toggleCompletion} handleEditPopup={handleEditPopup} handleDeletePopup={handleDeletePopup}/>
      )
    })
  }
  ...
  return (
    <div className="relative flex flex-col mr-3 w-full z-[20]">
        ...
        <DeleteHabitModal open={deleteModal} setOpen={setDeleteModal} setHabits={setHabits} i={currentIndex} id={id}/>
    </div>
  )
}
