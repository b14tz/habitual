...
export default function SetHabits({ habits, setHabits, habitError, setHabitError }) {
                ...
                if (habits[index]) {
                    return (
                        <div key={index} className="flex flex-row items-center">
                            <p className="mr-2">•</p>
                            <li>
                                <p className="font-semibold">{habits[index].title}</p>
                            </li>
                            {true ? (
    ...
    return (
        <div className="flex flex-col">
            ...
            />
            {habitError ? <p className="m-2 text-sm text-t-error">{habitError}</p> : null}
            <ul className="flex flex-col m-4">{renderList()}</ul>
        </div>
    );
}
